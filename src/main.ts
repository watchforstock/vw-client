...
const HEADERS_SESSION = {
  ...
  'User-Agent':
    'Mozilla/5.0 (Linux; Android 6.0.1; D5803 Build/23.5.A.1.291; wv) \
...
class Connection {
  ...
  public async update() {
    ...
    try {
      ...
      // // get vehicles
      let loadedVehicles = await this.post('-/mainnavigation/get-fully-loaded-cars', undefined, undefined);
      console.log(loadedVehicles);

      // // load all not loaded vehicles
      if (
        loadedVehicles.fullyLoadedVehiclesResponse &&
        loadedVehicles.fullyLoadedVehiclesResponse.vehiclesNotFullyLoaded
      ) {
        for (const vehicle of loadedVehicles.fullyLoadedVehiclesResponse.vehiclesNotFullyLoaded) {
          const vehicleVin = vehicle.vin;
          await this.post('-/mainnavigation/load-car-details/' + vehicleVin, undefined, undefined);
        }

        loadedVehicles = await this.post('-/mainnavigation/get-fully-loaded-cars', undefined, undefined);
      }
